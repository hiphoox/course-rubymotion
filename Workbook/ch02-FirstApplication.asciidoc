...
- Ability to select known Time Regions (local and remote)
...
[NOTE]
Please add the assets to the resources folder of your project. You can find them under the resources folder and inside there is an 'images' folder with the assets necessary for this chapter.  
...
class RootViewController < UIViewController
  ...
  def set_default_time
    ...
    @defaultTimeLabel.text = "UTC "+(defaultTimeZone.secondsFromGMT/3600).to_s + "  "+dateFormat
  end 
...
It’s time to add a control that lets us select from different time zones, open the *root_view_controller.rb* and add the following highlighted code: 
...
Next compile your application; you should see a big white button, don’t hesitate and try it.
...
Go ahead and look for a region that you may like to be and see the local time in this area. 

You‘ll never wonder again what time it is on other side of the planet, but you may think that it's not good enough to know the time in one region, two is always better than one. Let's add a second set of controls.

First of all open your *root_view_controller.rb* file and add the following highlighted code in the *loadView* method.
